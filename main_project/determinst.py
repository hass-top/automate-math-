...
class DeterministicCheckGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Automaton Deterministic Checker")
    ...
    def init_ui(self):
        ...
        # Check deterministic button
        self.check_button = QPushButton("Check Deterministic")
        self.check_button.clicked.connect(self.check_deterministic)
        self.check_button.setEnabled(False)
        left_layout.addWidget(self.check_button)

        # Convert to DFA button
        self.convert_button = QPushButton("Convert to DFA")
        self.convert_button.clicked.connect(self.convert_to_dfa)
        ...
        # Second visualization (DFA after conversion)
        ...
        right_layout.addWidget(QLabel("Converted DFA:"))
    ...
    def load_json(self):
        ...
        file_dialog.setNameFilter(f"Automaton files (*.aut)")
        ...
        if file_dialog.exec_():
            ...
            try:
                ...
                self.graph_label2.clear()  # Clear DFA panel
            except Exception as e:
                self.result_text.append(f"Error llloading JSON: {str(e)}")
                self.automate = None
                self.original_automate = None
                self.check_button.setEnabled(False)
                self.convert_button.setEnabled(False)
    ...
    def check_deterministic(self):
        ...
        try:
            ...
            self.result_text.append(f"\nDeterministic Check Result: The automaton is {'deterministic' if result else 'non-deterministic'}")
        except Exception as e:
            self.result_text.append(f"Error checking determinism: {str(e)}")
    ...
    def convert_to_dfa(self):
        if not self.automate:
            ...
            return
        #try:
        self.original_automate = self.automate  # Preserve original for visualization
        self.automate = afn_afd(self.automate)
        self.result_text.append("\nConverted to DFA")
        self.display_automaton_details()
        # Visualize both automata
        self.visualize_automaton(self.original_automate, self.graph_label1, "Original automaton visualized")
        self.visualize_automaton(self.automate, self.graph_label2, "DFA visualized")
